...
describe('Auth Routes', () => {
  ...
  describe('GET /api/auth/validate-reset-token/:token', () => {
    it('should return valid: true for valid token', async () => {
      ...
      const response = await request(app).get(`/api/auth/validate-reset-token/${resetToken}`);
    ...
    it('should return valid: false for invalid token', async () => {
      ...
      const response = await request(app).get('/api/auth/validate-reset-token/invalid-token');
