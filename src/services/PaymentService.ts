...
export class PaymentService {
  ...
  async createSubscription(data: {
    ...
    try {
      ...
      logger.info(
        `Subscription created for user ${data.userId}, plan ${data.planId}: ${response.id}`
      );
  ...
  async createPixPayment(data: {
    ...
    try {
      ...
      logger.info(
        `Creating Pix payment for user ${data.userId}, amount: ${data.amount}, email: ${data.payerEmail}`
      );
      ...
      logger.info(
        `Pix payment created for user ${data.userId}: ${response.id}, status: ${response.status}`
      );
  ...
  async createBoletoPayment(data: {
    ...
    try {
      ...
      logger.info(
        `Boleto payment created for user ${data.userId}: ${response.id}, status: ${response.status}`
      );
  ...
  verifyWebhookSignature(
    ...
    if (!webhookSecret) {
      logger.warn(
        '⚠️ MP_WEBHOOK_SECRET not set. Webhook signature verification DISABLED (dev only)'
      );
      return true;
    }
