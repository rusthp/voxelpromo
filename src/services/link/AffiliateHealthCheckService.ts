...
export class AffiliateHealthCheckService {
    ...
    private detectMarketplace(url: string): string {
        ...
        if (url.includes('aliexpress.com') || url.includes('s.click.aliexpress.com')) return 'aliexpress';
    ...
    private isProductUnavailable(html: string, marketplace: string): boolean {
        ...
        switch (marketplace) {
            ...
            case 'kabum':
                return (
                    lowerHtml.includes('produto não encontrado') ||
    ...
    async verifyLink(url: string, useCache: boolean = true): Promise<LinkHealthReport> {
        ...
        if (url.includes('your-affiliate-code') || url.includes('your-tracking-id') || url.includes('YOUR_')) {
            const result: LinkHealthReport = {
                url,
                isValid: false,
                statusCode: 0,
        ...
        for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
            try {
                ...
                if (attempt < this.maxRetries) {
                    ...
                    logger.debug(`[AffiliateHealthCheck] Retry ${attempt + 1}/${this.maxRetries} for ${url.substring(0, 50)}`);
                }
            }
        }
    ...
    async verifyBatch(urls: string[], concurrency: number = 5): Promise<LinkHealthReport[]> {
        ...
        for (let i = 0; i < urls.length; i += concurrency) {
            ...
            const batchResults = await Promise.all(
                batch.map(url => this.verifyLink(url))
            );
    ...
    async generateHealthReport(limit: number = 100): Promise<HealthCheckSummary> {
        try {
            ...
            logger.info(`[AffiliateHealthCheck] Report: ${healthy}/${results.length} links healthy (${summary.percentage}%)`);
