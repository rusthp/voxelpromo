...
export class UserSettingsService {
    ...
    async updateSettings(userId: string, payload: any): Promise<IUserSettings> {
        ...
        // Amazon
        if (payload.amazon) {
            settings.amazon.accessKey = updateIfNotMasked(settings.amazon.accessKey, payload.amazon.accessKey);
            settings.amazon.secretKey = updateIfNotMasked(settings.amazon.secretKey, payload.amazon.secretKey);
        ...
        if (payload.aliexpress) {
            settings.aliexpress.appKey = updateIfNotMasked(settings.aliexpress.appKey, payload.aliexpress.appKey);
            settings.aliexpress.appSecret = updateIfNotMasked(settings.aliexpress.appSecret, payload.aliexpress.appSecret);
            settings.aliexpress.trackingId = payload.aliexpress.trackingId ?? settings.aliexpress.trackingId;
            settings.aliexpress.isConfigured = !!(settings.aliexpress.appKey && settings.aliexpress.appSecret);
        }

        // Mercado Livre
        if (payload.mercadolivre) {
            settings.mercadolivre.clientId = payload.mercadolivre.clientId ?? settings.mercadolivre.clientId;
            settings.mercadolivre.clientSecret = updateIfNotMasked(settings.mercadolivre.clientSecret, payload.mercadolivre.clientSecret);
            settings.mercadolivre.redirectUri = payload.mercadolivre.redirectUri ?? settings.mercadolivre.redirectUri;
            settings.mercadolivre.affiliateCode = payload.mercadolivre.affiliateCode ?? settings.mercadolivre.affiliateCode;
            settings.mercadolivre.accessToken = updateIfNotMasked(settings.mercadolivre.accessToken, payload.mercadolivre.accessToken);
            settings.mercadolivre.refreshToken = updateIfNotMasked(settings.mercadolivre.refreshToken, payload.mercadolivre.refreshToken);
            settings.mercadolivre.tokenExpiresAt = payload.mercadolivre.tokenExpiresAt ?? settings.mercadolivre.tokenExpiresAt;
            settings.mercadolivre.sessionCookies = updateIfNotMasked(settings.mercadolivre.sessionCookies, payload.mercadolivre.sessionCookies);
            settings.mercadolivre.csrfToken = updateIfNotMasked(settings.mercadolivre.csrfToken, payload.mercadolivre.csrfToken);
            settings.mercadolivre.affiliateTag = payload.mercadolivre.affiliateTag ?? settings.mercadolivre.affiliateTag;
            settings.mercadolivre.isConfigured = !!(settings.mercadolivre.accessToken || settings.mercadolivre.affiliateCode);
        }
        ...
        if (payload.awin) {
            ...
            settings.awin.dataFeedApiKey = updateIfNotMasked(settings.awin.dataFeedApiKey, payload.awin.dataFeedApiKey);
        ...
        if (payload.shopee) {
            ...
            settings.shopee.isConfigured = !!(settings.shopee.feedUrls?.length || settings.shopee.affiliateCode);
        }

        // Telegram
        if (payload.telegram) {
            settings.telegram.botToken = updateIfNotMasked(settings.telegram.botToken, payload.telegram.botToken);
            settings.telegram.channelId = payload.telegram.channelId ?? settings.telegram.channelId;
            settings.telegram.isConfigured = !!(settings.telegram.botToken && settings.telegram.channelId);
        }

        // Instagram
        if (payload.instagram) {
            settings.instagram.appId = updateIfNotMasked(settings.instagram.appId, payload.instagram.appId);
            settings.instagram.appSecret = updateIfNotMasked(settings.instagram.appSecret, payload.instagram.appSecret);
            settings.instagram.accessToken = updateIfNotMasked(settings.instagram.accessToken, payload.instagram.accessToken);
            settings.instagram.pageAccessToken = updateIfNotMasked(settings.instagram.pageAccessToken, payload.instagram.pageAccessToken);
            ...
            settings.instagram.webhookVerifyToken = payload.instagram.webhookVerifyToken ?? settings.instagram.webhookVerifyToken;
            settings.instagram.isConfigured = !!(settings.instagram.accessToken && settings.instagram.igUserId);
        }

        // WhatsApp
        if (payload.whatsapp) {
            settings.whatsapp.enabled = payload.whatsapp.enabled ?? settings.whatsapp.enabled;
            settings.whatsapp.targetNumber = payload.whatsapp.targetNumber ?? settings.whatsapp.targetNumber;
            settings.whatsapp.targetGroups = payload.whatsapp.targetGroups ?? settings.whatsapp.targetGroups;
            settings.whatsapp.library = payload.whatsapp.library ?? settings.whatsapp.library;
            settings.whatsapp.isConfigured = !!(settings.whatsapp.enabled && (settings.whatsapp.targetNumber || settings.whatsapp.targetGroups?.length));
        }
        ...
        if (payload.x) {
            ...
            settings.x.accessTokenSecret = updateIfNotMasked(settings.x.accessTokenSecret, payload.x.accessTokenSecret);
            settings.x.oauth2ClientId = updateIfNotMasked(settings.x.oauth2ClientId, payload.x.oauth2ClientId);
            settings.x.oauth2ClientSecret = updateIfNotMasked(settings.x.oauth2ClientSecret, payload.x.oauth2ClientSecret);
            settings.x.oauth2RedirectUri = payload.x.oauth2RedirectUri ?? settings.x.oauth2RedirectUri;
            settings.x.oauth2AccessToken = updateIfNotMasked(settings.x.oauth2AccessToken, payload.x.oauth2AccessToken);
            settings.x.oauth2RefreshToken = updateIfNotMasked(settings.x.oauth2RefreshToken, payload.x.oauth2RefreshToken);
            settings.x.oauth2TokenExpiresAt = payload.x.oauth2TokenExpiresAt ?? settings.x.oauth2TokenExpiresAt;
            settings.x.oauth2Scope = payload.x.oauth2Scope ?? settings.x.oauth2Scope;
            settings.x.isConfigured = !!(settings.x.apiKey || settings.x.bearerToken || settings.x.oauth2AccessToken);
        }
        ...
        if (payload.ai) {
            ...
            settings.ai.openaiApiKey = updateIfNotMasked(settings.ai.openaiApiKey, payload.ai.openaiApiKey);
        ...
        if (payload.collectionSettings) {
            settings.collectionSettings = { ...settings.collectionSettings, ...payload.collectionSettings };
        }
    ...
    async disconnectIntegration(userId: string, integration: string): Promise<IUserSettings> {
        ...
        const resetMap: Record<string, () => void> = {
            ...
            whatsapp: () => { settings.whatsapp = { isConfigured: false, enabled: false }; },
            ...
            awin: () => { settings.awin = { isConfigured: false, enabled: false }; },
