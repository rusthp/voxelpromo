...
export class AliExpressScraper {
     ...
     */
    private extractPricesFromRunParams(data: any): { currentPrice: number; originalPrice: number; currency: string } | null {
        try {
            ...
            if (priceModule) {
                ...
                if (priceModule.minPrice) {
                    currentPrice = this.parsePrice(String(priceModule.minPrice.value || priceModule.minPrice));
                }
                if (priceModule.maxPrice) {
                    originalPrice = this.parsePrice(String(priceModule.maxPrice.value || priceModule.maxPrice));
                }
        ...
        try {
            ...
            if (runParams) {
                ...
                if (prices) {
                    ...
                    const discountPercentage = prices.originalPrice > prices.currentPrice
                        ? Math.round(((prices.originalPrice - prices.currentPrice) / prices.originalPrice) * 100)
                        : 0;

                    logger.info(`✅ Scraped product ${productId}: R$${prices.currentPrice} (was R$${prices.originalPrice})`);
     ...
     */
    private parseProductFromHtml(html: string, productId: string, url: string): ScrapedProduct | null {
        try {
            // Extract title
            ...
            if (priceMatches && priceMatches.length > 0) {
                const prices = priceMatches.map((p: string) => this.parsePrice(p)).filter((p: number) => p > 0).sort((a: number, b: number) => a - b);
                if (prices.length >= 2) {
            ...
            // Extract image
            const imageMatch = html.match(/<img[^>]*class="[^"]*magnifier[^"]*"[^>]*src="([^"]+)"/i) ||
    ...
    async scrapeBestDeals(limit: number = 20): Promise<ScrapedDealsPage> {
        try {
            ...
            const jsonDataMatch = html.match(/window\.__INITIAL_STATE__\s*=\s*(\{[\s\S]*?\});?\s*<\/script>/);
            ...
            if (products.length === 0) {
                ...
                const cardMatches = html.matchAll(/<div[^>]*class="[^"]*product-card[^"]*"[^>]*>([\s\S]*?)<\/div>(?=\s*<div[^>]*class="[^"]*product-card)/gi);
                ...
                for (const match of cardMatches) {
                    ...
                    if (priceMatches && priceMatches.length > 0) {
                        const prices = priceMatches.map((p: string) => this.parsePrice(p)).filter((p: number) => p > 0);
                        if (prices.length >= 1) {
                    ...
                    if (currentPrice > 0 && productId) {
                        products.push({
                            ...
                            currency: 'BRL',
                            imageUrl,
                            productUrl: productUrl.startsWith('http') ? productUrl : `https://pt.aliexpress.com${productUrl}`,
                            available: true,
                        });
                    }
    ...
    async scrapeBestsellersSSR(limit: number = 20): Promise<ScrapedDealsPage> {
        try {
            ...
            if (products.length === 0) {
                ...
                const productRegex = /"productId"\s*:\s*"?(\d+)"?\s*,[\s\S]*?"title"\s*:\s*"([^"]+)"[\s\S]*?"(?:price|salePrice)"\s*:\s*"?([0-9.]+)"?/g;
    ...
    private parseSSRProduct(item: any): ScrapedProduct | null {
        try {
            ...
            const currentPrice = this.parsePrice(String(item.price || item.salePrice || item.currentPrice || '0'));
            const originalPrice = this.parsePrice(String(item.originalPrice || item.marketPrice || currentPrice));
            ...
            return {
                ...
                productUrl: item.productUrl || item.url || `https://pt.aliexpress.com/item/${productId}.html`,
                available: true,
            };
        } catch {
            return null;
