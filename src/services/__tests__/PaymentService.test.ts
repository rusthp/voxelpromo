...
jest.mock('../../config/plans.config', () => ({
    getPlan: jest.fn((planId: string) => {
        const plans: Record<string, any> = {
            ...
            'pro-monthly': { id: 'pro-monthly', displayName: 'Performance', price: 4990, billingCycle: 'monthly', features: ['Filtros'] },
            'premium-monthly': { id: 'premium-monthly', displayName: 'Plus', price: 9990, billingCycle: 'monthly', features: ['Tudo'] }
        };
...
describe('PaymentService', () => {
    ...
    describe('verifyWebhookSignature', () => {
        ...
        it('should reject missing signature', () => {
            const result = service.verifyWebhookSignature(undefined, 'request-123', { data: { id: '123' } });
            expect(result).toBe(false);
        });

        it('should reject missing request ID', () => {
            const result = service.verifyWebhookSignature('ts=123,v1=abc', undefined, { data: { id: '123' } });
            expect(result).toBe(false);
        });

        it('should reject malformed signature format', () => {
            const result = service.verifyWebhookSignature('invalid-format', 'request-123', { data: { id: '123' } });
            expect(result).toBe(false);
        });
    ...
    describe('cancelSubscription', () => {
        ...
        it('should handle cancellation failure', async () => {
            ...
            await expect(service.cancelSubscription('sub-123'))
