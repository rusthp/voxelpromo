...
export const filterOffersSchema = Joi.object({
    ...
    sort: Joi.string().valid('createdAt', '-createdAt', 'discountPercentage', '-discountPercentage').default('-createdAt'),
});

export const scheduleOfferSchema = Joi.object({
    offerIds: Joi.array().items(Joi.string().pattern(/^[0-9a-fA-F]{24}$/)).min(1).required(),
    scheduledFor: Joi.date().iso().greater('now').required(),
    platforms: Joi.array().items(Joi.string().valid('telegram', 'x', 'whatsapp')).min(1).required(),
});

export const postOffersSchema = Joi.object({
    offerIds: Joi.array().items(Joi.string().pattern(/^[0-9a-fA-F]{24}$/)).min(1).max(10).required(),
    platforms: Joi.array().items(Joi.string().valid('telegram', 'x', 'whatsapp')).min(1).required(),
});
